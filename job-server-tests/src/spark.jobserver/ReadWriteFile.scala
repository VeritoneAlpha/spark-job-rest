 ...
 */
object ReadWriteFile extends SparkJob {
  ...
  override def runJob(sc: SparkContext, config: Config): Any = {
    ...
    val inputFile = config.getString("input.string")

    val textFileRdd = sc.textFile(inputFile);
    textFileRdd.count();
    textFileRdd.saveAsTextFile(inputFile + ".output.txt");
  }
}
