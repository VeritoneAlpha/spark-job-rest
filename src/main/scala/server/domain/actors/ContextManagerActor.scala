...
class ContextManagerActor(defaultConfig: Config) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    case CreateContext(contextName, jars, config) => {
      ...
      if(contextMap contains contextName) {
        ...
        val processBuilder = createProcessBuilder(contextName, port, jars, mergedConfig, jmxProperty)
  ...
  def createProcessBuilder(contextName: String, port: Int, jars: String, config: Config, jmxProperty: String): ProcessBuilder = {
    ...
    val pb = new ProcessBuilder(scriptPath, jars, contextName, port.toString, xmxMemory, jmxProperty)
