          ...
          if (!contextName.head.isLetter) {
            ...
            parameterMap { (params) =>
              ...
              if(!myMutableMap.get("spark.ui.port").isDefined){
                port = findAvailablePort().toString
                logger.info("spark.ui.port was not defined. Setting it to port : " + port )
                myMutableMap.put("spark.ui.port", port)
              } else {
              ...
              respondWithMediaType(MediaTypes.`application/json`) { ctx =>
                future.map {
                  case ContextInitialized   => ctx.complete(StatusCodes.OK, port)
