...
class ContextManagerActor(defaultConfig: Config, jarActor: ActorRef) extends Actor {
          ...
          case Success(value) => {
            ...
            value match {
              case result:ResultJarsPathForAll => {
                ...
                val processBuilder = createProcessBuilder(contextName, port, result.pathForClasspath, mergedConfig, jmxProperty)
  ...
  def addSparkExecutorJMXPortToConfig(config: Config, jmxPort: Int): Config = {
    ...
    val configValue = ActorUtils.createJmxConfigValue(jmxPort + jmxExecutorMinusDriver)
    map.put(sparkExtraJavaOptsPath, configValue)
    val newConf = ConfigFactory.parseMap(map)
    newConf.withFallback(config)
  }

  def createProcessBuilder(contextName: String, port: Int, jarsForClasspath: String, config: Config, jmxProperty: String): ProcessBuilder = {
    ...
    val pb = new ProcessBuilder(scriptPath, jarsForClasspath, contextName, port.toString, xmxMemory, jmxProperty)
